...
impl QcowRawFile {
    ...
    /// compute every entry into a local buffer first, then perform a single
    /// positioned write of the whole table. We must never keep a pending
    ...
    ///
    /// Uses a single positioned write for the same reason as
...
mod pointer_table_offset_tests {
    ...
    fn temp_path(tag: &str) -> std::path::PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        p.push(format!("ch_ptr_table_{tag}_{nanos}.qcow_raw"));
        p
    }

    fn be_bytes(entries: &[u64]) -> Vec<u8> {
        let mut v = Vec::with_capacity(entries.len() * size_of::<u64>());
        for e in entries {
    ...
    fn make_qcow_raw(path: &std::path::Path) -> QcowRawFile {
        {
            let mut f = OpenOptions::new()
                ...
                .open(path)
                .unwrap();
            f.write_all(&vec![0u8; FILE_LEN as usize]).unwrap();
            f.flush().unwrap();
        }
        ...
        QcowRawFile::from(raw, CLUSTER_SIZE, 16).expect("QcowRawFile::from")
    }
    ...
    fn write_pointer_table_lands_at_offset_despite_callback_seek() {
        let path = temp_path("wpt");
        let mut qcow = make_qcow_raw(&path);
        let entries: Vec<u64> = vec![0x1111_2222_3333_4444u64; 8]; // 64 bytes

        qcow
            .write_pointer_table(TARGET_OFFSET, entries.iter(), |q, addr| {
        ...
        let mut verify = OpenOptions::new().read(true).open(&path).unwrap();
        let mut whole = Vec::new();
        verify.read_to_end(&mut whole).unwrap();
        let found_at = find_all(&whole, &expected);
        eprintln!(
            "table bytes found at offsets {:x?}; TARGET_OFFSET={:#x}",
            found_at, TARGET_OFFSET
        );
    ...
    fn write_pointer_table_direct_lands_at_offset() {
        let path = temp_path("wptd");
        let mut qcow = make_qcow_raw(&path);
        let entries: Vec<u64> = vec![0xAAAA_BBBB_CCCC_DDDDu64; 8];

        qcow
            .write_pointer_table_direct(TARGET_OFFSET, entries.iter())
